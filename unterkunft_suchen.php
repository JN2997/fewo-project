...
if (!empty($personen) && !empty($land)) {
    ...
    if (!empty($filters)) {
	 ...
	 // Überprüfen, ob keine Häuser den Kriterien entsprechen
    if (empty($haus_ids)) {
        // Ausgabe einer Fehlermeldung, wenn keine Häuser gefunden wurden
        echo '<p>Leider entspricht kein Ferienhaus Ihren Wünschen, bitte versuchen Sie es erneut.</p>';
    } else {
    ...
    foreach ($haus_ids as $haus_id) {
        ...
        echo '<a href="unterkunft_details.php?HAUS_ID=' . htmlspecialchars($haus['HAUS_ID'], ENT_QUOTES, 'UTF-8') . '" class="button">Mehr Infos und Buchen</a>';
        echo '</div>';
        echo '</div>';
    }
	}
